...
### Where Mock Data Must Be Replaced

| Frontend File | Mock Function/Data | Backend Endpoint |
|--------------|-------------------|-----------------|
| `useSimulationStore.ts` → `generateMockQualifying()` | `simulationMockData.ts` | `POST /api/qualifying/` |
| `useSimulationStore.ts` → `generateMockRaceData()` | `simulationMockData.ts` | `POST /api/simulate/` |
| `useDashboardStore.ts` → `mockDrivers` | 22 inline drivers | `GET /api/standings/drivers` |
| `useDashboardStore.ts` → `mockConstructors` | 11 inline teams | `GET /api/standings/constructors` |
| `useDashboardStore.ts` → `mockCalendar` | 23-race calendar | `GET /api/calendar/2026` |
| `useDashboardStore.ts` → `mockInsights` | Strategy data | `GET /api/insights/{circuit}` |
| `mocks/drivers.ts` | Static driver list | `GET /api/drivers/` |
| `mocks/circuits.ts` | Static circuit list | `GET /api/circuits/` |

### Simulation Flow (Backend Must Support)

```
1. User selects Circuit → GET /api/circuits/{id} (lap count, etc.)
2. Weekend Intro → No API call
