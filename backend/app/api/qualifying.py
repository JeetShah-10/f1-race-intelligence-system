...
def _simulate_qualifying_lap(model, driver, team, circuit, session):
    ...
    # Apply Qualifying Mode Modifiers
    # - Low fuel (~10kg vs 110kg race) -> approx 3s faster
    # - Maximum engine mode -> 0.5s faster
    # - DRS always available -> accounted for in model? Model trained on race pace?
    # ML Model is trained on RACE pace. Quali is usually ~3-4s faster.
    
    quali_adjustment = -3.5
    
    # Add random variance (driver error, track evolution)
    # Q3 track evolution: faster than Q1
    evolution = {"Q1": 0.0, "Q2": -0.2, "Q3": -0.4}[session]
    
    # Driver variance: +/- 0.3s
